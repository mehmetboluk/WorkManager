...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val data = Data.Builder().putInt("myData", 1).build()
        ...
        WorkManager.getInstance(this).getWorkInfoByIdLiveData(myWorkRequest.id).observe(this,
            Observer {
                ...
                if(it.state == WorkInfo.State.RUNNING) println("Running")
                else if(it.state == WorkInfo.State.FAILED) println("Failed")
                else if (it.state == WorkInfo.State.SUCCEEDED) println("Succeeded")
            })

        //WorkManager.getInstance(this).cancelAllWork() iptal etmek icin
